...
def search_case_law(keywords: str) -> str:
    """Find a leading case by keyword for common contract and tort issues.

    Args:
        keywords: Search keywords for the relevant issue.
    """
    cases = {
...
QUESTION = (
    "A software vendor willfully breached a supply contract with a tech startup that has "
    "$5M in annual revenue. What contract remedies, leading case law, estimated exposure, "
    "and compliance risks should the startup evaluate?"
)

SYSTEM_PROMPT = (
    "You are a legal analyst agent. You have access to tools for searching legal databases, "
    "finding leading case law, calculating penalties, and checking compliance requirements. "
    "For breach-of-contract questions, search the legal database and case law before answering. "
    "Use the penalty and compliance tools only when they materially help the analysis. "
...
async def main():
    ...
    llm = get_llm()
    # This LangGraph version exposes `debug=True` instead of `verbose=True`.
